...
void Pow2Solver::checkInitialRefine()
{
  ...
  for (const Node& i : d_pow2s)
  {
    ...
    // positive x >= 0 -> pow2(x) > 0
    Node xgeq0 = nm->mkNode(GEQ, i[0], d_zero);
    Node nonegative = nm->mkNode(GT, i , d_zero);
    conj.push_back(nm->mkNode(IMPLIES, xgeq0, nonegative));

    // even: x>0 -> pow2(x) mod 2 = 0
    Node xgt0 = nm->mkNode(GT, i[0], d_zero);
    Node mod2 = nm->mkNode(INTS_MODULUS, i , d_two);
    Node even = nm->mkNode(EQUAL, mod2 , d_zero);
    conj.push_back(nm->mkNode(IMPLIES, xgt0, even));

    // // lower bound 0: 0 <= x -> x < pow2(x)
    // Node xltpow2x = nm->mkNode(LT, i[0], i);
    // conj.push_back(nm->mkNode(IMPLIES, xgeq0, xltpow2x));
    
    // // lower bound 1: x > 2 -> pow2(x) > x+x+1
...
void Pow2Solver::checkFullRefine()
{
  ...
  for (uint64_t i = 0, size = d_pow2s.size(); i < size; i++)
  {
    ...
    for (uint64_t j = i + 1; j < size; j++)
    {
      ...
      // strong monotonicity
      if (x >= 0 && x < y &&  pow2y <= pow2x)
      {
        Node x_lt_y = nm->mkNode(LT, n[0], m[0]);
        Node xgeq0 = nm->mkNode(LEQ, d_zero, n[0]);
        Node assumption = nm->mkNode(AND, xgeq0, x_lt_y);
        Node conclusion = nm->mkNode(LT, n, m);
        Node lem = nm->mkNode(IMPLIES, assumption, conclusion);
        d_im.addPendingLemma(
            lem, InferenceId::ARITH_NL_POW2_MONOTONE_REFINE, nullptr, true);
      } else if (y < x &&  pow2x <= pow2y)
      {
        Node assumption = nm->mkNode(LT, m[0], n[0]);
        Node conclusion = nm->mkNode(LT, m, n);
        Node lem = nm->mkNode(IMPLIES, assumption, conclusion);
        d_im.addPendingLemma(
            lem, InferenceId::ARITH_NL_POW2_MONOTONE_REFINE, nullptr, true);
      } 
    ...
    // div 0
    if(x >= 0 && x > pow2x) {
        Node assumption = nm->mkNode(GEQ, n[0], d_zero);
        Node div_zero = nm->mkNode(Kind::INTS_DIVISION, n[0], n);
        Node conclusion = nm->mkNode(EQUAL, div_zero, d_zero);
        Node lem = nm->mkNode(IMPLIES, assumption, conclusion);
        d_im.addPendingLemma(
            lem, InferenceId::ARITH_NL_POW2_TRIVIAL_CASE_REFINE, nullptr, true);
    }

    // lower bound 2: k > 5 means x > k => pow2(x) > kx + k^2
    if (x >= 7 && pow2x <= x*x*2) { 
      ...
      Node assumption = nm->mkNode(AND, x_gt_k, k_gt_5);
    ...
    // integer lemmas: pow2(x) = 0 whenever x < 0
    if (x < 0 && pow2x != 0)
    {
