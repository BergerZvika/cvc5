...
void PIAndSolver::checkInitialRefine()
{
  ...
  for (const std::pair<const Node, std::vector<Node> >& is : d_piands)
  {
    ...
    for (const Node& i : is.second)
    {
      ...
      Node assum_max_x = nm->mkNode(Kind::AND, k_gt_0, x_modpow2_eq_max, y_range);
      ...
      conj.push_back(nm->mkNode(Kind::IMPLIES, arg1Mod2_eq_zero, piand_mod_two.eqNode(d_zero)));
      // lsb lemmas: y mod 2 = 0 => piand(k,x,y) % 2 = 0
      Node arg0Mod2_eq_zero = nm->mkNode(Kind::EQUAL, arg0Mod2, d_zero);
      conj.push_back(nm->mkNode(Kind::IMPLIES, arg0Mod2_eq_zero, piand_mod_two.eqNode(d_zero)));
  ...
  for (const std::pair<Node, std::vector<Node> >& is : d_piands)
  {
    ...
    for (const Node& i : is.second)
    {
      ...
      if (model_k > 0 && model_x == 1 && model_y == 1 && model_piand != 1) {
          ...
          d_im.addPendingLemma(
              xy_one_lem, InferenceId::ARITH_NL_PIAND_BASE_CASE_REFINE, nullptr, true);
      }
      ...
      for (const Node& n : is.second)
      {
            ...
            if(j > index) {
              ...
              if (model_k > 0 && model_k == model_k2 && model_x != model_x2 && model_y == model_y2 && model_piand == model_x2 && model_piand2 == model_x) {
                Node noneqx = nm->mkNode(Kind::AND, k.eqNode(k2), (x.eqNode(x2)).notNode(), y.eqNode(y2));
                Node ranges_assum = nm->mkNode(Kind::AND, x_range, x2_range, y_range);
                Node assum_difference = nm->mkNode(Kind::AND, k_gt_0, noneqx, ranges_assum);
                Node difference = nm->mkNode(Kind::OR, i.eqNode(x2).notNode(), n.eqNode(x).notNode());
                Node diff_lemm = nm->mkNode(Kind::IMPLIES, assum_difference, difference);
                d_im.addPendingLemma(
                  diff_lemm, InferenceId::ARITH_NL_PIAND_DIFFERENCE_REFINE, nullptr, true);
              }
              // symmetry: piand(k,x,y) = piand(k,y,x)
              if (model_k == model_k2 && model_x == model_y2 && model_x2 == model_y && model_piand != model_piand2) {
                Node assum_sym = nm->mkNode(Kind::AND, k.eqNode(k2), (x.eqNode(y2)), y.eqNode(x2));
                Node sym_lemm = nm->mkNode(Kind::IMPLIES, assum_sym,  i.eqNode(n));
                d_im.addPendingLemma(
                  sym_lemm, InferenceId::ARITH_NL_PIAND_SYMETRY_REFINE, nullptr, true);
              }
            }
        } 
        ...
        if (model_x + model_y == max_int && model_piand != 0) {
          ...
          d_im.addPendingLemma(
                  neg_lem, InferenceId::ARITH_NL_PIAND_CONTRADITION_REFINE, nullptr, true);
        }
...
Node PIAndSolver::valueBasedLemma(Node i)
{
  ...
  valC = rewrite(valC);
  Node lem = nm->mkNode(
      Kind::IMPLIES, nm->mkNode(Kind::AND, k.eqNode(valK), x.eqNode(valX), y.eqNode(valY)), i.eqNode(valC));
  return lem;
}
...
Node PIAndSolver::sumBasedLemma(Node i, Kind kind)
{
  ...
  Node then = nm->mkNode(kind, i, d_iandUtils.createSumNode(x, y, int_k, granularity));
